      ...
      <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {SITE.programs.map((program, i) => {
          ...
          return (
            <article
              key={program.key}
              className={`reveal${i > 0 ? ` d${Math.min(i, 3)}` : ""} group flex flex-col overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--m-canvas)] shadow-[0_1px_2px_rgba(16,24,40,0.04)] transition-[transform,box-shadow] duration-200 hover:-translate-y-1 hover:shadow-[0_18px_40px_-20px_rgba(16,24,40,0.25)] motion-reduce:transition-none motion-reduce:hover:translate-y-0`}
            >
